...
class ReviewAdapter(val list: List<ReviewResponse>, val context: Context) : RecyclerView.Adapter<ReviewAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        holder.name.text = list[position].customer.email
...
//        holder.img.setImageResource(list[position].img)
      ...
      /*  holder.img.setOnClickListener {
        ...
        var date=list[position].created_at
        holder.date.text = date.substring(0,10)

        var rating=list[position].rate
